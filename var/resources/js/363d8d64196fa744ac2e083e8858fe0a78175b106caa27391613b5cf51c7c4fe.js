...
function disableInputsInForm(form){var inputs=jQuery(form).find(':input');if(inputs.length)_.each(inputs,function(input){$(input).attr('disabled',true)})}
function submitForm(form,attrs){jQuery.each(attrs,function(name,value){var e=form.elements.namedItem(name);if(e)e.value=value});var disableInputs=_.partial(disableInputsInForm,form);if(form.commonController){var valid=form.commonController.validate({silent:true});if(!valid)disableInputs=function(){}};jQuery(form).submit();_.delay(disableInputs,100)}
function submitFormDefault(form,action){var attrs={};if(action!==null)attrs.action=action;submitForm(form,attrs)};
/* AUTOGENERATED: activate_key.js */
function PopupButtonActivateKey(){PopupButtonActivateKey.superclass.constructor.apply(this,arguments)};extend(PopupButtonActivateKey,PopupButton);PopupButtonActivateKey.prototype.pattern='.activate-key';PopupButtonActivateKey.prototype.enableBackgroundSubmit=false;PopupButtonActivateKey.prototype.callback=function(selector,link){PopupButton.prototype.callback.apply(this,arguments);jQuery('form',selector).each(function(){this.commonController.backgroundSubmit=false})};core.autoload(PopupButtonActivateKey);core.microhandlers.add('PopupButtonActivateKey',PopupButtonActivateKey.prototype.pattern,function(event){core.autoload(PopupButtonActivateKey)});;
/* AUTOGENERATED: upgrade.js */
...
/* AUTOGENERATED: controller.js */
function QueryMarketplace(){core.bind('forceMpAction',_.bind(this.forceMpAction,this));QueryMarketplace.superclass.constructor.apply(this,arguments);this.queryMarketplace()};extend(QueryMarketplace,ALoadable);QueryMarketplace.prototype.pattern='#queryMarketplace';QueryMarketplace.prototype.maxAttepmts=1;QueryMarketplace.prototype.completed=false;QueryMarketplace.prototype.forceMpActions=false;QueryMarketplace.prototype.forceMpAction=function(event,state){this.forceMpActions=true;if(this.completed)this.queryMarketplace()};QueryMarketplace.prototype.isNeedQuery=function(base){return(this.forceMpActions||0<core.getCommentedData(base,'hasPendingActions').length)&&0<base.length&&!base.hasClass('active')&&!base.hasClass('processed')};QueryMarketplace.prototype.queryMarketplace=function(){var base=jQuery(this.pattern);if(this.isNeedQuery(base)){this.scheduleQuery(base)}else this.completed=true};QueryMarketplace.prototype.scheduleQuery=function(base){var obj=this;setTimeout(function(){base.addClass('active').removeClass('error');var suffix='';if(core.getCommentedData(base,'parentTarget').length)suffix='&parentTarget='+core.getCommentedData(base,'parentTarget');jQuery.ajax({url:xliteConfig.script+"?target=marketplace&action=update"+suffix,async:true,dataType:'json',cache:false,global:false}).done(function(data,textStatus){base.addClass('processed');obj.processSuccess(data)}).fail(function(xhr,textStatus,error){base.addClass('error');this.maxAttepmts-=1;if(0<this.maxAttepmts)setTimeout(this.queryMarketplace,5e3)}).always(function(){base.removeClass('active');this.completed=true})},1e3)};QueryMarketplace.prototype.processSuccess=function(data){if(data&&0<data.actions.length)for(i=0;i<data.actions.length;i++)core.trigger('mp_event_'+data.actions[i],data)};core.autoload(QueryMarketplace);;
/* AUTOGENERATED: controller.js */
function LazyLoadWidget(base){this.callSupermethod('constructor',arguments);this.widgetClass=jQuery(base).data('lazyClass');jQuery(_.bind(this.load,this,{}));this.lazyEvent=jQuery(base).data('lazyEvent');if(this.lazyEvent){var events=this.lazyEvent.split(',');for(i=0;i<events.length;i++){var e=events[i].trim();core.bind(e,_.bind(this.handleLazyEvent,this))}}};extend(LazyLoadWidget,ALoadable);LazyLoadWidget.prototype.reloadIfError=false;LazyLoadWidget.prototype.shadeWidget=false;LazyLoadWidget.prototype.isScheduledReload=false;LazyLoadWidget.prototype.getEventNamespace=function(){return'lazy-load-widget'};LazyLoadWidget.autoload=function(){jQuery('.lazy-load-widget.active').each(function(){new LazyLoadWidget(this)})};LazyLoadWidget.prototype.handleLazyEvent=function(event,data){var obj=this;obj.isScheduledReload=true;setTimeout(function(){if(obj.isScheduledReload){obj.load();obj.isScheduledReload=false}},1e3)};core.autoload('LazyLoadWidget');;
/* AUTOGENERATED: notifications.js */
jQuery(function(){core.microhandlers.add('updateMenuNotificationCount','.notification-menu',function(){var $self=jQuery(this);if(0<$self.data('unread-count')&&!$self.parent('.box').is('.read')){var node=$self.closest('.box').parent();node.addClass('has-unread');node.find('.icon .unread-mark').text($self.data('unread-count'))}});core.microhandlers.add('assignPopupHandlersOnNotifications','.notification-menu',function(){core.autoload(PopupButton)});jQuery('body').on('mouseenter','.menu.notification',function(){var element=jQuery(this);if(!element.is('.has-unread'))return;var menuType=jQuery('.notification-menu',element).data('menu-type');this.readTimer=setTimeout(function(){jQuery.cookie('notification_read_'+menuType,Date.now());element.removeClass('has-unread');jQuery.ajax({url:xliteConfig.script+"?target=main&action=set_notifications_as_read&menuType="+menuType}).done(function(){jQuery.removeCookie('notification_read_'+menuType)})},2e3)});jQuery('body').on('mouseleave','.menu.notification',function(){clearInterval(this.readTimer)})});;
/* AUTOGENERATED: header_search.js */
core.microhandlers.add('header-search','.header-search',function(){var base=jQuery(this);base.find('.dropdown-menu li a').click(function(){var a=jQuery(this);base.find('input[name="substring"]').attr('placeholder',a.data('placeholder'));base.find('input[name="code"]').val(a.data('code'));base.find('input[name="substring"]').click().focus();return false})});;
/* AUTOGENERATED: storefront_status.js */
var StorefrontStatusView=function(base){base=base||jQuery('#header .storefront-status');Base.apply(this,[base]);this.base=base;this.blocked=false;this.initialize()};extend(StorefrontStatusView,Base);StorefrontStatusView.prototype.initialize=function(){this.base.find('a.toggler').click(_.bind(function(event){var result=true;if(!this.blocked&&this.userConfirm()){this.blocked=true;jQuery(this.base).addClass('disabled');result=core.get(jQuery(event.currentTarget).attr('href'))};return!result},this));core.bind('switchstorefront',_.bind(this.handleSwicthStorefront,this))};StorefrontStatusView.prototype.userConfirm=function(){var toggler=this.base.find('.toggler'),result=true;if(toggler.hasClass('on'))result=confirm(core.t('Do you really want to close storefront?'));return result};StorefrontStatusView.prototype.handleSwicthStorefront=function(event,data){var toggler=this.base.find('.toggler');if((data.opened&&toggler.hasClass('off'))||(!data.opened&&toggler.hasClass('on')))this.switchState();if(data.link)toggler.attr('href',data.link);if(data.privatelink)this.base.find('a.link.closed').attr('href',data.privatelink);jQuery(this.base).removeClass('disabled');this.blocked=false};StorefrontStatusView.prototype.switchState=function(){var toggler=this.base.find('.toggler');if(toggler.hasClass('off')){toggler.removeClass('off').addClass('on');this.base.removeClass('closed').addClass('opened')}else{toggler.removeClass('on').addClass('off');this.base.removeClass('opened').addClass('closed')}};core.autoload('StorefrontStatusView');;
/* AUTOGENERATED: controller.js */
function LeftMenu(){var self=this;this.accordion=true;this.$menu=jQuery('#leftMenu');this.$body=jQuery('body');this.scrollTop=window.scrollY;jQuery(window).scroll(_.partial(_.bind(this.recalculatePosition,this),undefined)).resize(_.partial(_.bind(this.recalculatePosition,this),undefined));this.$menu.on('affix-top.bs.affix',function(){jQuery(this).css('top',0)});jQuery('.left-menu-ctrl').bind('click',_.bind(this.toggleMenu,this));jQuery('.menu .link',this.$menu).filter(function(){return jQuery(this).parent().nextAll('.box').length>0}).each(function(){var item=jQuery(this).closest('li'),box=jQuery(this).parent().nextAll('.box'),height=box.outerHeight();if(box.parents('.compressed').length){box.css('width',198);height=box.outerHeight()-60;box.css('width','')};box.css('height',height);item.data('box-height',height);box.css('visibility','visible');box.css('position','relative');item.addClass(item.hasClass('pre-expanded')?'expanded':'collapsed').removeClass('pre-expanded');setTimeout(function(){box.css('transition','opacity .25s ease-in-out, height .25s ease-in-out')},300)}).bind('click',function(e){if(self.$menu.hasClass('compressed'))return true;e.preventDefault();self.toggleItem(jQuery(this).closest('li'));return false});jQuery('body').on('mouseenter','#leftMenu.compressed > ul > li',function(){var box=jQuery('.box',this);if(box.length)self.correctPosition(jQuery('.box',this))});this.recalculatePosition();core.bind('recalculateLeftMenuPosition',function(){self.recalculatePosition()})};LeftMenu.prototype.getWindowHeight=function(){return window.innerHeight};LeftMenu.prototype.getScrollDelta=function(){var result=window.scrollY-this.scrollTop;this.scrollTop=window.scrollY;return result};LeftMenu.prototype.getMenuHeight=function(){var result=0;this.$menu.children().each(function(){result+=jQuery(this).outerHeight()});return result};LeftMenu.prototype.getHeaderSpace=function(){var headerSpace=jQuery('#header-wrapper').outerHeight()-window.scrollY;headerSpace=headerSpace>0?headerSpace:0;if($('.demo-header-wrapper').length>0)headerSpace+=$('.demo-header-wrapper').outerHeight();return headerSpace};LeftMenu.prototype.recalculatePosition=function(heightDelta){if(document.body.scrollTop+window.innerHeight>document.body.scrollHeight)return;var viewPortHeight=window.innerHeight,headerSpace=this.getHeaderSpace();this.$menu.css('min-height',viewPortHeight-headerSpace);var menuHeight=this.getMenuHeight()+(heightDelta||0),sideBar=jQuery('#sidebar-first');sideBar.height(menuHeight);var scrollDelta=this.getScrollDelta(),viewPortTop=window.scrollY,menuTop=this.$menu.offset().top,viewPortBottom=viewPortTop+viewPortHeight,menuBottom=menuTop+menuHeight,menuCssTop=parseInt(this.$menu.css('top')),offset=menuCssTop-scrollDelta,maxOffset=menuHeight-viewPortHeight,isAffix=this.$menu.is('.affix')&&menuTop!==sideBar.offset().top&&menuTop!==0;if(isAffix)if(menuHeight<viewPortHeight){this.$menu.css('min-height',viewPortHeight);this.$menu.css('top',0)}else{if(scrollDelta>0&&viewPortBottom<menuBottom){this.$menu.css('top',-offset<maxOffset?offset:-maxOffset)}else if(scrollDelta<0&&viewPortTop>menuTop)this.$menu.css('top',offset<0?offset:0);if(viewPortBottom>menuBottom)this.$menu.css('top',menuCssTop+(viewPortBottom-menuBottom));var body=document.body,html=document.documentElement,height=Math.max(body.scrollHeight,body.offsetHeight,html.clientHeight,html.scrollHeight,html.offsetHeight);if(menuBottom>=height){var self=this;setTimeout(function(){var menuHeight=self.getMenuHeight();if(menuTop+menuHeight>=height)self.$menu.animate({top:viewPortHeight-menuHeight},150)},300)}};if(this.$menu.is('.compressed')){var visibleBox=jQuery('.box',this.$menu).filter(function(){return jQuery(this).css('visibility')==='visible'});if(visibleBox.length)this.correctPosition(visibleBox)}};LeftMenu.prototype.windowScroll=function(){};LeftMenu.prototype.windowResize=function(){};LeftMenu.prototype.toggleItem=function(element){var delta=0;if(element.hasClass('expanded')){this.hideItem(element);delta-=element.data('box-height')}else{if(this.accordion){var self=this;jQuery('.menu-item',this.$menu).each(function(){var item=jQuery(this);if(item.hasClass('expanded'))delta-=item.data('box-height');self.hideItem(item)})};this.showItem(element);delta+=element.data('box-height')};this.recalculatePosition(delta)};LeftMenu.prototype.hideItem=function(element){element.removeClass('expanded').addClass('collapsed');core.trigger('layout.sidebar.changeHeight')};LeftMenu.prototype.showItem=function(element){element.addClass('expanded').removeClass('collapsed');core.trigger('layout.sidebar.changeHeight')};LeftMenu.prototype.toggleMenu=function(){if(this.$body.hasClass('left-menu-compressed')){this.decompress()}else this.compress();return false};LeftMenu.prototype.compress=function(){var box=jQuery('.menu div.box',this.$menu);box.hide();setTimeout(function(){box.show()},250);this.$menu.addClass('compressed');this.$body.addClass('left-menu-compressed');jQuery.cookie('XCAdminLeftMenuCompressed',1);core.trigger('left-menu-compressed')};LeftMenu.prototype.decompress=function(){this.$menu.removeClass('compressed');this.$body.removeClass('left-menu-compressed');jQuery('.box',this.$menu).css('top',0);jQuery.cookie('XCAdminLeftMenuCompressed',0)};LeftMenu.prototype.correctPosition=function(box){box.css('top',0);var boxTop=box.offset().top,boxBottom=boxTop+box.outerHeight(),viewportTop=window.scrollY,viewportBottom=viewportTop+document.documentElement.offsetHeight;if(boxBottom>(viewportBottom-10)){box.css('top',viewportBottom-boxBottom-20)}else if(boxTop<(viewportTop+10))box.css('top',viewportTop-boxTop)};core.autoload(LeftMenu);;
/* AUTOGENERATED: controller.js */
function PopupButtonSalesChannels(){PopupButtonSalesChannels.superclass.constructor.apply(this,arguments);var self=this;jQuery('.menu-item.sales-channels:not(:has(.box))>.line>span.link').click(function(){self.base.click()})};extend(PopupButtonSalesChannels,PopupButton);PopupButtonSalesChannels.prototype.pattern='.action-widget .sales-channels';core.autoload(PopupButtonSalesChannels);;
/* AUTOGENERATED: controller.js */
core.microhandlers.add('ExtensionsNode','#leftMenu .extensions',function(){$('body').tooltip({selector:'.compressed .extensions .link',placement:'right',container:'body'})})
function PopupButtonHotAddons(){PopupButtonHotAddons.superclass.constructor.apply(this,arguments)};extend(PopupButtonHotAddons,PopupButton);PopupButtonHotAddons.prototype.pattern='.action-widget .extensions';core.autoload(PopupButtonHotAddons);;
/* AUTOGENERATED: switcher-standalone.js */
function ButtonSwitcherStandalone(base){this.base=base;var self=this;jQuery(this.base).change(function(event){event.stopImmediatePropagation();self.executeShaded(function(onSuccess){self.runAction(onSuccess)})})};ButtonSwitcherStandalone.prototype.executeShaded=function(callback){event.stopImmediatePropagation();var switchWrapper=jQuery(this.base);assignShadeOverlay(switchWrapper);callback(function(){unassignShadeOverlay(switchWrapper)});return false};ButtonSwitcherStandalone.prototype.runAction=function(onSuccess){var data=core.getCommentedData(this.base);core.get(data.url,onSuccess,data.data);return false};core.microhandlers.add('standalone-switch','.standalone-switch',function(event){new ButtonSwitcherStandalone(this)});;